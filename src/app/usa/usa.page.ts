...
export class USAPage implements OnInit{
    ...
    this.httpnative.get(this.performanceAPIUrl,{},{}).then((data)=>{
      ...
      this.httpnative.get(this.APIUrl,{},{}).then((data)=>{
        ...
        for(let data of this.somedata){
          if(JSON.stringify(data.indexName).indexOf("U.S.")!== -1)
          {
            this.usa[this.count] = data.indexName;
  ...
  {
    let that=this;
      this.indexdetails = this.somedata.filter((item)=> item.indexName === that.tem);
      ...
      console.log("before")
      this.performancedetails = this.anotherdata.filter((item)=> item.indexName === that.tem);
